            ...
            _self.listTypeShow = [[
                ...
                }},
                {
                    ...
                    formatter: function (value, row, index) {
                        var btn = "";
                        btn ='<a class="smallBtn" target="_self" href="abnormal_fence/map_no_fence_update.html?id="'+(row.id)+'>修改</a> &nbsp;'
...
function fenceDelete(id,status) {
    layer.confirm('您真的确定要'+(status==0?'停用':'启用')+'该围栏吗？', {icon: 3, title: '提示'}, function (index) {
        return $.ajax({
            url: "http://" + window.location.host + "/sichuang-wangyueche/abnormal/changeStatus",
            dataType: "json",
